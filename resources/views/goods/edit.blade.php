...
<form class="form-horizontal" role="form" action="{{url('goods/update/'.$goods->goods_id)}}" method="post" enctype="multipart/form-data">
	...
	<div class="form-group">
		...
		<div class="col-sm-8">
			<input type="text" class="form-control goods_name" name="goods_name" value="{{$goods->goods_name}}" id="firstname" 
				   placeholder="请输入商品名字">
	...
	<div class="form-group">
		...
		<div class="col-sm-8">
			<input type="text" class="form-control goods_price" name="goods_price" value="{{$goods->goods_price}}" id="firstname" 
				   placeholder="请输入商品价格">
	...
	<div class="form-group">
		...
		<div class="col-sm-8">
			<input type="text" class="form-control goods_num" name="goods_num" value="{{$goods->goods_num}}" id="firstname" 
				   placeholder="请输入商品库存">
	...
	<div class="form-group">
		...
		<div class="col-sm-8">
			...
			<img src="{{env('UPLOAD_URL')}}{{$goods->goods_img}}" alt="" width="100">
		</div>
	</div>
	<div class="form-group">
		...
		<div class="col-sm-8">
			...
			@if($goods->goods_imgs)
                    @php $photos = explode('|',$goods->goods_imgs); @endphp
	...
	<div class="form-group">
        ...
        <input type="radio" name="is_new" id="optionsRadios3" value="1" {{$goods->is_new==1?'checked':''}}> 是
   		</label>
		<label class="radio-inline">
		<input type="radio" name="is_new" id="optionsRadios3" value="2" {{$goods->is_new==2?'checked':''}}> 否
   		</label>
	</div>
	<div class="form-group">
		<label for="firstname" class="col-sm-2 control-label">是否精品</label>
		<label class="radio-inline">
        <input type="radio" name="is_best" id="optionsRadios3" value="1" {{$goods->is_best==1?'checked':''}}> 是
   		</label>
		<label class="radio-inline">
		<input type="radio" name="is_best" id="optionsRadios3" value="2" {{$goods->is_best==2?'checked':''}}> 否
   		</label>
	</div>
	<div class="form-group">
		<label for="firstname" class="col-sm-2 control-label">是否热卖</label>
		<label class="radio-inline">
        <input type="radio" name="is_hot" id="optionsRadios3" value="1" {{$goods->is_hot==1?'checked':''}}> 是
   		</label>
		<label class="radio-inline">
		<input type="radio" name="is_hot" id="optionsRadios3" value="2" {{$goods->is_hot==1?'checked':''}}> 否
   		</label>
	</div>
	<div class="form-group">
		<label for="firstname" class="col-sm-2 control-label">是否上架</label>
		<label class="radio-inline">
        <input type="radio" name="is_up" id="optionsRadios3" value="1" {{$goods->is_up==1?'checked':''}}> 是
   		</label>
		<label class="radio-inline">
		<input type="radio" name="is_up" id="optionsRadios3" value="2" {{$goods->is_up==2?'checked':''}}>否
   		</label>
	</div>
	<div class="form-group">
		...
		<div class="col-sm-8">
			<select name="b_id" id="">
				...
				@foreach($brand as $v)
				<option value="{{$v->b_id}}" {{$goods->b_id==$v->b_id?'selected':''}}>{{$v->b_name}}</option>
				@endforeach
			</select>
		</div>
	</div>
	<div class="form-group">
		...
		<div class="col-sm-8">
			<select name="c_id" id="">
				<option value="">--请选择--</option>
				@foreach($cate as $v)
					<option value="{{$v->c_id}}" {{$goods->c_id==$v->c_id?'selected':''}}>
